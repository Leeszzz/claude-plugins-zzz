...
1. **语言**：所有输出使用中文
2. **客观性与真实性**：
   - **严格来源**：所有总结内容必须完全来源于论文原文。
   - **禁止解读**：严禁过度解读、自行编写或加入模型幻觉。
   - **不存在处理**：如果论文中不包含模板要求的某项内容（如实验设置不详或未提及未来方向），**必须直接在相应位置注明“论文中未提及”或“不存在”**，不得空缺也不得编造。
3. **结构性**：严格按照模板格式输出
4. **简洁性**：Brief 模式的 abstract 控制在 200 字以内
5. **完整性**：Detailed 模式尽量填充所有章节，根据上述准则处理缺失信息。
